...
export const Profile: React.FC = () => {
  ...
  useEffect(() => {
    async function loadReviews() {
      ...
      try {
        const targetId = user?.id || 'usr-demo-iyyanar';
        const res = await ratingApi.getUserReviews(targetId);
        if (res.success && res.data) {
  ...
  const filteredReviews = reviewFilterStar
    ? reviews.filter((r) => Math.round(r.rating) === reviewFilterStar)
    : reviews;
      ...
      <div className="bg-white rounded-3xl border border-slate-100 p-6 sm:p-8 shadow-sm">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6">
          ...
          <button
            ...
            className="flex items-center gap-2 px-5 py-3 bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-bold rounded-2xl shadow-md shadow-indigo-100 transition-all active:scale-95 flex-shrink-0 cursor-pointer"
          >
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mt-8 pt-6 border-t border-slate-100">
          ...
          <button
            ...
            className="p-4 rounded-2xl bg-amber-50/40 border border-amber-100/50 text-left hover:bg-amber-50/70 transition-all cursor-pointer"
          >
            <div className="flex items-center justify-between">
            ...
            <div className="py-16 text-center flex flex-col items-center justify-center">
              ...
              <p className="text-xs text-slate-500 max-w-sm mb-4">
                {searchQuery
                  ...
                  : 'Publish your pre-owned items to reach thousands of buyers across SwapIt.'}
              </p>
              {searchQuery ? (
                <button
                  type="button"
                  ...
                  >
                    ...
                    <div className="flex items-center gap-2 self-end lg:self-auto flex-wrap">
                      <button
                        ...
                        className="px-3 py-2 text-xs font-semibold text-slate-600 hover:text-indigo-600 bg-slate-50 hover:bg-indigo-50 rounded-xl transition-all flex items-center gap-1.5 border border-slate-200 cursor-pointer"
                      ...
                      <button
                        ...
                        className="px-3 py-2 text-xs font-semibold text-indigo-700 bg-indigo-50 hover:bg-indigo-100 rounded-xl transition-all flex items-center gap-1.5 border border-indigo-100 cursor-pointer"
                      ...
                      <button
                        ...
                        className={`px-3 py-2 text-xs font-semibold rounded-xl transition-all flex items-center gap-1.5 cursor-pointer ${
                          isSold
        ...
        <div className="bg-white rounded-3xl border border-slate-100 p-6 sm:p-8 shadow-sm space-y-8">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2.5">
              ...
              <div>
                <h2 className="text-lg font-black text-slate-900 tracking-tight">
                  Your Seller Reputation & Feedback
                </h2>
                <p className="text-xs text-slate-400">
                  Detailed ratings and testimonials from completed buyer deals
                </p>
              </div>
            </div>
          </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-5 gap-6 p-6 rounded-2xl bg-gradient-to-br from-slate-50 to-indigo-50/30 border border-slate-200/60">
              ...
              <div className="md:col-span-2 flex flex-col items-center justify-center text-center sm:border-r border-slate-200/60 pr-4">
                ...
                <span className="text-xs text-slate-500 font-semibold">
                  Overall rating across {ratingSummary.totalReviews} reviews
                </span>

                <div className="mt-4 flex flex-wrap justify-center gap-1.5">
                  {ratingSummary.topTags.map((t, idx) => (
                    <span
                      key={idx}
                      className="text-[10px] bg-white border border-slate-200 text-indigo-700 px-2.5 py-1 rounded-full font-bold shadow-2xs"
                    >
                      ✓ {t.tag} ({t.count})
                    </span>
                  ))}
              ...
              <div className="md:col-span-3 flex items-center pl-2">
                <RatingBreakdown
                  breakdown={ratingSummary.breakdown}
                  percentages={ratingSummary.breakdownPercentages}
          ...
          {/* Reviews List Toolbar & Filter */}
          <div className="space-y-4">
            <div className="flex items-center justify-between flex-wrap gap-3">
              <h3 className="text-xs font-bold text-slate-700 uppercase tracking-wider">
                Buyer Testimonials ({filteredReviews.length})
              </h3>
              ...
              <div className="flex items-center gap-1.5 overflow-x-auto py-1">
                <button
                  type="button"
                  onClick={() => setReviewFilterStar(null)}
                  className={`text-xs px-3 py-1.5 rounded-xl font-bold transition-all cursor-pointer ${
                    reviewFilterStar === null
                ...
                {[5, 4, 3, 2, 1].map((s) => (
                  <button
                    key={s}
                    type="button"
                    onClick={() => setReviewFilterStar(reviewFilterStar === s ? null : s)}
                    className={`text-xs px-2.5 py-1.5 rounded-xl font-bold flex items-center gap-1 transition-all cursor-pointer ${
                      reviewFilterStar === s
            ...
            {reviewsLoading ? (
              ...
              <div className="text-center py-12 bg-slate-50 rounded-2xl border border-slate-100">
                ...
                <h4 className="text-xs font-bold text-slate-800">No reviews found</h4>
                <p className="text-[11px] text-slate-400 mt-0.5">
                  {reviewFilterStar
                    ? `No ${reviewFilterStar}-star reviews received yet.`
                    : 'Complete more swap deals and sales to gather buyer reputation!'}
                </p>
              </div>
            ) : (
              <div className="space-y-3">
